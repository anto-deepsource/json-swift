...
extension JSValue {
    ...
    for codeunit in generator {
      if codeunit.isWhitespace() {
        ...
        throw JsonParserError(
          code: ErrorCode.ParsingError.code, domain: JSValueErrorDomain, userInfo: info)
      }
    }
  }
  ...
  static func parse(_ generator: ReplayableGenerator) throws -> JSValue {
    for codeunit in generator {
      ...
      } else if codeunit.isDigit() || codeunit == Token.Minus || codeunit == Token.Plus
        || codeunit == Token.Period
      {
  ...
  static func parseNumber(_ generator: ReplayableGenerator) throws -> JSValue {
    ...
    loop: for (idx, codeunit) in generator.enumerated() {
      ...
      case (_, Token.Zero...Token.Nine, .leadingZero):
        ...
        throw JsonParserError(
          code: ErrorCode.ParsingError.code, domain: JSValueErrorDomain, userInfo: info)
      case (_, Token.One...Token.Nine, .leadingNegativeSign), (_, Token.One...Token.Nine, .initial):
      ...
      case (_, Token.Zero...Token.Nine, .fractional):
        ...
        fractionMultiplier *= 10
      case (_, Token.e, .leadingZero), (_, Token.E, .leadingZero), (_, Token.e, .integer),
        (_, Token.E, .integer), (_, Token.e, .fractional), (_, Token.E, .fractional):
        state = .exponent
      ...
      case (_, Token.Zero...Token.Nine, .exponent), (_, Token.Zero...Token.Nine, .exponentSign),
  ...
  static func contextualString(_ generator: ReplayableGenerator, left: Int = 5, right: Int = 10)
    -> String
  {
    var string = ""
